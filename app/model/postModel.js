...
exports.insertData = (data, cb) => { 
    bcrypt.genSalt(saltRound, (err, salt) => { //bycrypt.gensalt : 자동으로 랜덤값 받기
        if (err) throw new Error(err); 
        bcrypt.hash(data.password, salt, (err, hash) => { //hash화 해주기
            if (err) throw new Error(err); 
            
            // 입력 구문 
            let now = new Date(); //let : 변수 선언 (선언과 초기화 분리해서 선언)
            let sql = 'INSERT INTO posts (name, email, password, subject, content, ip, created_at) VALUES (?, ?, ?, ?, ?, inet_aton(?), ?)'; 
            let bindParam = [ 
                data.name, 
                data.email,
                hash, // 해싱된 비밀번호 
                data.subject, data.content, data.ip, now 
            ]; 
            // 참고사이트 
 ...
 */
  ...
  */
  ...
  exports.updateData = (data, cb) => {
      //해시된 비밀번호를 찾아온다.(현재 비밀번호와 맞는지 비교)
              ...
              bcrypt.compare(data.password, hash_password, (err,result) => {
                  ...
                  if(result) {
                    ...
                    mysqlConn.query(sql, bindParam, (err,results, fields) => {
                      ...
                      //비밀번호 대조에서 결과가 틀리다면 false를 리턴한다.
                      cb(false);
                  }
              });
          } else {
              //해시된 기존 비밀번호를 찾지 못할 경우 false를 리턴한다.
              cb(false);
          }
      });
  };
       ...
       mysqlConn.query(sql, [data.id], (err, results, fields) => {
           ...
           if(hash_password){
               ...
               bcrypt.compare(data.password, hash_password, (err, result) => {
                   ...
                   //결과는 true, false로 나온다.
                   if(result){
                       let sql = 'DELETE FROM posts WHRER id=?';
                       mysqlConn.query(sql, [data.id], (err, results, fields) => {
                           if(err) throw new Error(err);
